...
const getApplicationTitle = (row?: PaymentHistoryRow | null) => {
    return asString(row?.application_details?.title ?? row?.raw?.application_details?.title, '-')
}

const getPaymentAmount = (row?: PaymentHistoryRow | null) => {
    return asNumber(row?.amount ?? row?.total_amount ?? row?.installment?.amount ?? row?.application_details?.amount)
}

const getPaymentStatus = (row?: PaymentHistoryRow | null) => {
    return asString(row?.status ?? row?.installment_status ?? row?.account_management_status ?? row?.application_status, '-')
}

const normalizeRow = (item: Record<string, unknown>, index: number): PaymentHistoryRow => {
    const row = item as PaymentHistoryRow
    const accountId = asString(row.account_management_id ?? row.id, String(index + 1))
    const applicationNo = asString(row.application_no ?? row.application_details?.application_no)
...
const normalizeApiRows = (response: unknown): PaymentHistoryRow[] => {
    if (Array.isArray(response)) {
        return response.filter(asRecord).map((item, index) => normalizeRow(item, index))
    }
    ...
    if (asRecord(response)) {
        ...
        if (Array.isArray(envelope.data)) {
            return envelope.data.filter(asRecord).map((item, index) => normalizeRow(item, index))
        }
        ...
        if (asRecord(envelope.data)) {
            ...
            if (Array.isArray(nestedEnvelope.data)) {
                return nestedEnvelope.data.filter(asRecord).map((item, index) => normalizeRow(item, index))
            }
        }
    }
...
const extractMeta = (response: unknown): PaymentHistoryResponse['meta'] | null => {
...
const StatusCell = (params: ICellRendererParams<PaymentHistoryRow, string | null | undefined>) => {
...
const MemberCell = (params: ICellRendererParams<PaymentHistoryRow, string | null | undefined>) => {
    ...
    return (
        <div className='flex  min-w-0 flex-col justify-center '>
            ...
            <span className='truncate text-xs -mt-5 font-medium text-muted-foreground'>
                {memberId}
            </span>
        </div>
    )
...
const ApplicationCell = (params: ICellRendererParams<PaymentHistoryRow, string | null | undefined>) => {
    ...
    return (
        <div className='flex min-w-0 flex-col justify-center'>
            ...
            <span className='truncate text-xs -mt-5 text-muted-foreground'>
                {applicationTitle}
            </span>
        </div>
    )
...
const TextCell = (params: ICellRendererParams<PaymentHistoryRow, string | null | undefined>) => {
    return (
...
const DateCell = (params: ICellRendererParams<PaymentHistoryRow, string | null | undefined>) => {
    return (
        <div className='flex h-full items-center'>
...
const PaymentModeCell = (params: ICellRendererParams<PaymentHistoryRow, string | null | undefined>) => {
    return (
        <div className='flex h-full items-center'>
            <Badge
                variant='secondary'
        ...
        <div className='w-full overflow-hidden rounded-xl border bg-background'>
            ...
            <div className='divide-y'>
                {Array.from({ length: rows }).map((_, rowIndex) => (
                    <div key={`row-${rowIndex}`} className='grid grid-cols-7 gap-4 px-4 py-4'>
...
const PaymentHistory = () => {
    const [selectedRow, setSelectedRow] = React.useState<PaymentHistoryRow | null>(null)
    const [viewOpen, setViewOpen] = React.useState(false)
    const [quickFilter, setQuickFilter] = React.useState('')
    const [statusFilter, setStatusFilter] = React.useState<PaymentHistoryStatusFilter>('pending')
    const [gridApi, setGridApi] = React.useState<GridApi<PaymentHistoryRow> | null>(null)
    const [exportMenuOpen, setExportMenuOpen] = React.useState(false)
    const exportMenuRef = React.useRef<HTMLDivElement | null>(null)

    const { data, isLoading, isError, error } = useQuery({
        queryKey: ['payment-history', statusFilter],
        queryFn: () => getPaymentHistoryHandler({ status: statusFilter }),
    })
        ...
        () => [
            {
                ...
                minWidth: 145,
                sort: 'desc',
                valueFormatter: (params: ValueFormatterParams<PaymentHistoryRow, string | null | undefined>) => formatDateOnly(params.value),
            ...
            },
            {
                ...
                minWidth: 270,
                flex: 1.1,
                valueGetter: (params) => `${getMemberName(params.data)} ${getMemberId(params.data)}`,
            ...
            },
            {
                ...
                minWidth: 250,
                flex: 1,
                valueGetter: (params) => `${getApplicationNo(params.data)} ${getApplicationTitle(params.data)}`,
            ...
            },
            {
                ...
                valueFormatter: (params: ValueFormatterParams<PaymentHistoryRow, number>) => `₹ ${formatCurrency(params.value)}`,
    ...
    const handleExportExcel = React.useCallback(() => {
        ...
        const workbookHtml = `
            <html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel">
                ...
                <body>
                    ...
                    <p>Status: ${escapeHtml(statusFilter)} | Rows: ${exportRows.length}${meta?.total !== undefined ? ` | Total: ${meta.total}` : ''}</p>
                    ${tableHtml}
                </body>
            </html>
        `
        ...
        link.href = url
        link.download = `payment-history-${statusFilter}.xls`
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
        URL.revokeObjectURL(url)
        setExportMenuOpen(false)
    }, [getExportTableHtml, statusFilter, exportRows.length, meta?.total])
    ...
    const handleGridReady = React.useCallback((event: GridReadyEvent<PaymentHistoryRow>) => {
    ...
    return (
        <>
            <style jsx global>{`
                ...
                .payment-history-grid .ag-body-horizontal-scroll-viewport::-webkit-scrollbar,
                .payment-history-grid .ag-body-vertical-scroll-viewport::-webkit-scrollbar {
                    display: none !important;
                    width: 0 !important;
                    height: 0 !important;
                }
            ...
            <div className='w-full space-y-4 overflow-hidden rounded-md'>
                <div>
                    ...
                    <p className='mt-1 text-xs text-muted-foreground'>
                        Review member payments, installment approvals, references, and payment modes.
                    </p>
                </div>
                ...
                <div className='overflow-hidden rounded-xl border bg-background shadow-sm'>
                        ...
                        <div className='flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between'>
                            <div className='grid w-full grid-cols-1 gap-3 md:grid-cols-[minmax(240px,1fr)_170px] xl:max-w-2xl'>
                                ...
                                <div>
                                    ...
                                    <Select
                                        value={statusFilter}
                                        onValueChange={(value) => setStatusFilter(value as PaymentHistoryStatusFilter)}
                                    >
                    ...
                    <div className='payment-history-grid h-160 w-full overflow-hidden'>
                        <AgGridReact<PaymentHistoryRow>
                            ...
                            getRowId={(params) => params.data.rowKey ?? String(params.data.id ?? Math.random())}
