...
import { createNoticeHandler, updateNoticeHandler } from "@/services/noticeHandler"
...
type NoticeFormError = {
    message?: string
    errors?: Record<string, string[] | string>
}

type EditNoticeDialogProps = {
    open: boolean
...
function getErrorMessage(
    error: NoticeFormError | null,
    field: keyof NoticeFormState | "created_by_admin_id" | "updated_by_admin_id" | "attachment"
) {
...
function mapNoticeToForm(notice?: NoticeItem | null): NoticeFormState {
    if (!notice) {
                ...
                <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild>
                        <Button
                            ...
                            variant="outline"
                            className={cn(
                                "h-10 justify-start rounded-lg px-3 text-left text-sm font-normal",
...
export default function EditNoticeDialog({
    ...
    const [form, setForm] = React.useState<NoticeFormState>(DEFAULT_FORM_STATE)
    const [formError, setFormError] = React.useState<NoticeFormError | null>(null)
    ...
    const createNoticeMutation = useMutation({
        mutationFn: async (payload: NoticeFormState) => {
    ...
    const updateNoticeMutation = useMutation({
        mutationFn: async (payload: NoticeFormState) => {
            ...
            return await updateNoticeHandler(notice.id, {
                title: payload.title.trim(),
                audience: payload.audience,
                message: payload.message.trim(),
                publish_at: payload.publish_at,
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (mode === "edit") {
            updateNoticeMutation.mutate(form)
            return
        }
            ...
            <DialogContent className="w-[95vw] lg:max-w-2xl gap-0 overflow-hidden rounded-lg p-0">
                ...
                <form onSubmit={handleSubmit} className="space-y-5 px-5 py-5">
                    <div className="space-y-1.5">
                        ...
                        />
                        {getErrorMessage(formError, "title") ? <p className="text-xs text-destructive">{getErrorMessage(formError, "title")}</p> : null}
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-1.5">
                            ...
                            {getErrorMessage(formError, "audience") ? <p className="text-xs text-destructive">{getErrorMessage(formError, "audience")}</p> : null}
                        </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        />
                        {getErrorMessage(formError, "message") ? <p className="text-xs text-destructive">{getErrorMessage(formError, "message")}</p> : null}
                    </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        />
                        {getErrorMessage(formError, "attachment") ? <p className="text-xs text-destructive">{getErrorMessage(formError, "attachment")}</p> : null}
                    </div>
