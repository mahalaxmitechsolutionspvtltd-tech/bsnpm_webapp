...
import { createSanchalakHandler, CreateSanchalakInput, SanchalakStatus } from "@/services/sanchalakHandlers"

type CreateSanchalakDialogProps = {
  open: boolean
...
export function CreateSanchalakDialog({ open, onOpenChange }: CreateSanchalakDialogProps) {
  const queryClient = useQueryClient()
  const [previewUrl, setPreviewUrl] = React.useState<string>("")
  const [serverError, setServerError] = React.useState<string>("")

  const createMutation = useMutation({
    mutationFn: createSanchalakHandler,
    onSuccess: async () => {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const payload: CreateSanchalakInput = {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-h-[92vh] overflow-y-auto rounded-lg sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Add Sanchalak</DialogTitle>
          <DialogDescription>
            Create a new sanchalak profile with basic login details and optional profile photo.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-4 rounded-lg border border-dashed p-4 sm:flex-row sm:items-center">
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <p className="text-xs text-muted-foreground">Optional image. JPG, PNG or WEBP below 15MB.</p>
