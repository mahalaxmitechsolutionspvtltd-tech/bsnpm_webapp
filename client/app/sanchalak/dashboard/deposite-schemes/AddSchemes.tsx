...
import { addDepositeSchemeHandler } from "@/services/depositeHandler"
import type { CreateDepositSchemePayload } from "@/types/depositeTypes"
...
export function AddSchemes() {
    ...
    const addSchemeMutation = useMutation({
        mutationFn: addDepositeSchemeHandler,
        onSuccess: async () => {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            const payload: CreateDepositSchemePayload = {
                scheme_name: parsed.data.scheme_name,
                created_by: user?.admin_name ?? "Admin",
                ...(parsed.data.scheme_details ? { scheme_details: parsed.data.scheme_details } : {}),
                ...(parsed.data.term_notes ? { term_notes: parsed.data.term_notes } : {}),
                ...(parsed.data.investment_terms ? { investment_terms: parsed.data.investment_terms } : {}),
                ...(parsed.data.interest_rate
                    ? { interest_rate: Number(parsed.data.interest_rate) }
                    : {}),
            }

            await addSchemeMutation.mutateAsync(payload)
        },
    })
    ...
    return (
        ...
        >
            ...
            <DialogContent className="overflow-hidden p-0 sm:max-w-3xl rounded-xl">
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <div className="space-y-1">
                            ...
                            <DialogDescription className="text-sm text-muted-foreground">
                                Add a new deposit scheme with rate, details and terms.
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
                                ...
                                return (
                                    <Field>
                                        ...
                                        <Textarea
                                            ...
                                            className="min-h-23 rounded-xl resize-none"
                                        />
                                        {errors.length > 0 ? <FieldError errors={toFieldErrorObjects(errors)} /> : null}
                                    </Field>
                                )
                ...
                <DialogFooter className="border-t px-6 py-4 sm:justify-end">
                    <DialogClose asChild>
                        <Button
                            ...
                            disabled={addSchemeMutation.isPending}
                        >
                            Cancel
                        </Button>
                    </DialogClose>
                    ...
                    >
                        {addSchemeMutation.isPending ? <Spinner /> : "Save Scheme"}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
